...
class TestNativeStructure(unittest.TestCase):
    ...
    def test_bezctx_structure(self):
        ...
        for fieldname, fieldtype in _context.bezctx._fields_:
            ...
            # so we settle for checking their repr() instead. Note that
            # different platforms seem to have different conventions for this
            # repr()! Fortunately, all the ones I've seen look like
            # "^<class 'ctypes.[...]CFunctionType'>".
            self.assertRegex(repr(fieldtype),
                             r"^<class 'ctypes\.[^' ]*CFunctionType'>$")
            self.assertIsInstance(fieldtype, Callable)
...
class TestSVGPathContext(unittest.TestCase):
    ...
    def test_lineto(self):
        ...
        self.assertEqual(self.buffer.getvalue(), 'L-314,159 ')
